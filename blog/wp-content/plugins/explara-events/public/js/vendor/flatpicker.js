...
function FlatpickrInstance(element, config) {
	...
	function createDay(className, date, dayNumber, i) {
		...
		if (dateIsEnabled) {
			...
			if (self.selectedDates[0] && date > self.minRangeDate && date < self.selectedDates[0]) self.minRangeDate = date;
			else if (self.selectedDates[0] && date < self.maxRangeDate && date > self.selectedDates[0]) self.maxRangeDate = date;
		}
	...
	function buildDays(delta) {
		...
		if (!self.config.animate || delta === undefined) clearNode(self.daysContainer);
		else {
		...
		if (delta >= 0) self.daysContainer.appendChild(dayContainer);
		else self.daysContainer.insertBefore(dayContainer, self.daysContainer.firstChild);
				...
				case "ArrowRight":
					if (!isTimeObj) {
						...
						if (self.daysContainer) {
							...
							if (!e.ctrlKey) focusOnDay(e.target.$i, _delta);
							else changeMonth(_delta, true);
	...
	function onMouseOver(elem) {
		...
		var _loop = function _loop(timestamp, i) {
			...
			if (initialDate < hoverDate && timestamp === initialDate.getTime()) dayElem.classList.add("startRange");
			else if (initialDate > hoverDate && timestamp === initialDate.getTime()) dayElem.classList.add("endRange");
	...
	function minMaxDateSetter(type) {
		return function(date) {
			...
			if (self.daysContainer) {
				...
				if (isValidDate) self.currentYearElement[type] = dateObj.getFullYear();
	...
	function selectDate(e) {
		...
		if (self.config.mode === "single") self.selectedDates = [selectedDate];
			...
			if (selectedIndex) self.selectedDates.splice(selectedIndex, 1);
		...
		// maintain focus
		if (!shouldChangeMonth) focusOnDay(e.target.$i, 0);
		else afterDayAnim(function() {
	...
	function set(option, value) {
		if (option !== null && (typeof option === "undefined" ? "undefined" : _typeof(option)) === "object") _extends(self.config, option);
	...
	function setSelectedDate(inputDate, format) {
		...
		else if (inputDate instanceof Date || !isNaN(inputDate)) self.selectedDates = [self.parseDate(inputDate, format)];
	...
	function parseDateRules(arr) {
		for (var i = arr.length; i--;) {
			if (typeof arr[i] === "string" || +arr[i]) arr[i] = self.parseDate(arr[i], null, true);
	...
	function onMonthNavClick(e) {
		...
		} else if (e.target.className === "arrowUp") self.changeYear(self.currentYear + 1);
		else if (e.target.className === "arrowDown") self.changeYear(self.currentYear - 1);
	}
	...
	parseDate: function parseDate(date, givenFormat, timeless) {
		...
		if (date instanceof Date) date = new Date(date.getTime()); // create a copy
			...
			if (date === "today") {
				...
				date = new Date(date);
			else if (this.config && this.config.parseDate) date = this.config.parseDate(date, format);
			else {
				(function() {
...
function flatpickr(selector, config) {
	...
	else if (!(selector instanceof HTMLElement)) return _flatpickr(window.document.querySelectorAll(selector), config);
